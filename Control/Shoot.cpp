...
f BP::BP_deal(u8 YK_Mode, u8 jianshu_flag)
{
    if (YK_Mode == SHOOT_MODE)
    {
        if (UD_BP_ON.updata(bp->ONE_ON) == UpDown_check_rising && mcl->Mode == 1)
        {
            ...
            {
                //bp->One_Target_Angle -= BOPAN_ANGLE + ((int32_t)M2006_BP.mang_inf % (int32_t)BOPAN_ANGLE);
            }
            else
            {
        ...
        {
            bp->Continuous_shooting_flag = 1;
            bp->Continuous_Target_Angle += Shot_SP_1 * 12; //(1s/9)
        ...
        // if (UD_PR_BP.updata(YK.yaogan.ch0 < 600) == UpDown_check_rising)
        // {
        //     bp->Continuous_Target_Angle = ((int32_t)M2006_BP.mang_inf / (int32_t)BOPAN_ANGLE) * (int32_t)BOPAN_ANGLE + BOPAN_ANGLE;
        // }
    }
    else if (YK_Mode == PLAYER_MODE)
    {
        remain_heat = shooter_id1_17mm_cooling_limit - shooter_id1_17mm_cooling_heat;
        if (UD_BP_DF.updata(YK.shubiao.press_l) == UpDown_check_rising && (remain_heat > 30 || YK.Pressed_Check(KEY_PRESSED_CTRL)) ) //&& (remain_heat > 30 || YK.Pressed_Check(KEY_PRESSED_CTRL))
        {
            bp->One_Target_Angle = M2006_BP.mang_inf;
            bp->One_Target_Angle += BOPAN_ANGLE;
            bp->Continuous_Target_Angle = bp->One_Target_Angle;
        ...
        else if ((((YK.shubiao.press_l) && bp->delay_2ms > 150) || (request.zimiao_status && YK.shubiao.press_l && SuperPower.mode == 2 && mcl->Mode)) && buff_mode == 0)
        {
            bp->Continuous_shooting_flag = 1;
#if BP_TEST_FLAG
            bp->Continuous_Target_Angle += Shot_SP_1 * 7; //(1s/9)
#else
            ...
            else if (remain_heat >= 30 && remain_heat < 60)
            {
            ...
            {
                bp->Continuous_Target_Angle += Shot_SP_1 * 8;
                bp->Prefabricate_Flag = 1;
            }
            else if (remain_heat >= 120 && remain_heat < 150)
            {
                bp->Continuous_Target_Angle += Shot_SP_1 * 9;
                bp->Prefabricate_Flag = 1;
            }
            else if (remain_heat >= 150 && remain_heat < 190)
            {
                bp->Continuous_Target_Angle += Shot_SP_1 * 11;
                bp->Prefabricate_Flag = 1;
            }
            else if (remain_heat >= 190 && remain_heat < 220)
            {
                bp->Continuous_Target_Angle += Shot_SP_1 * 13;
                bp->Prefabricate_Flag = 1;
            }
        ...
        // if ((UD_PR_BP.updata(YK.shubiao.press_l) && bp->Prefabricate_Flag))
        // {
        //     bp->Prefabricate_Flag = 0;
        //     bp->Continuous_Target_Angle = ((int32_t)M2006_BP.mang_inf / (int32_t)BOPAN_ANGLE) * (int32_t)BOPAN_ANGLE + BOPAN_ANGLE;
        // }
    }
    if ((bp->Mode == GYRO_MODE || ((jianshu_flag || bp->Mode == AUTO_MODE))))
    {
        if (bp->Continuous_shooting_flag)
        {
            M2006_BP_Con_Angle.PID_new_update(bp->Continuous_Target_Angle, M2006_BP.mang_inf);
            M2006_BP_Con_Speed.PID_new_update(M2006_BP_Con_Angle.OUT_PID, M2006_BP.sp);
            bp->PID_OUT = M2006_BP_Con_Speed.OUT_PID;
        }
        else
        {
            M2006_BP_Angle.PID_new_update(bp->One_Target_Angle, M2006_BP.mang_inf);
            M2006_BP_Speed.PID_new_update(M2006_BP_Angle.OUT_PID, M2006_BP.sp);
            bp->PID_OUT = M2006_BP_Speed.OUT_PID;
        }
    }
    else
        bp->PID_OUT = 0;
